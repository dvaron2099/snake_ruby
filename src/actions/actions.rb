module Actions
    def self.move_snake(state)
        ...
        #verificar que la casilla sea valida
        ...
        else                                                        #si no es valida -> terminar el juego
            end_game(state)                                             #si es valida -> movemos la serpiente
        end
    end
    ...
    def self.gererate_food(state)
        # rand
        new_food = Model::Food.new(rand(state.grid.row), rand(state.grid.col)) #rows cols
        state.food = new_food
        state
    end
    ...
    def self.position_is_valid?(state, position)
        ...
        is_invalid = ((position.row >= state.grid.row ||        #rows
            position.row < 0) ||
            (position.col >= state.grid.col ||                  #cols
            position.col < 0))
